...
# combination & svd
def cal_extrinsic_from_two_box_object_list(box_object_list1, box_object_list2, filter_num = 15, verbose = False, using_cpp_version = False, true_matches = None, optimize_combination = True, VIPS_flag = False):
    if using_cpp_version:
        matches_with_score_list = get_matches_with_score_py(box_object_list1, box_object_list2)
    else:
        boxes_match = BoxesMatch(box_object_list1, box_object_list2, VIPS_flag=VIPS_flag)
    ...
    for infra_file_name, vehicle_file_name, infra_boxes_object_list, vehicle_boxes_object_list, T_true in wrapper:
        ...
        if True:
            ...
            result = cal_extrinsic_from_two_box_object_list(filtered_infra_boxes_object_list, filtered_vehicle_boxes_object_list, filter_num=filter_num, verbose=verbose,
                                                             using_cpp_version=using_cpp_version, true_matches=available_matches, optimize_combination=False, VIPS_flag=True)
...
if __name__ == '__main__':
    batching_test_extrisic_from_two_box_object_list(verbose = False, filter_num = 15, using_predict_score=False, using_cpp_version=False,
                                                    output_dict='intermediate_output/VIPS-non_extrinsic_optimization/hard_dataset')
