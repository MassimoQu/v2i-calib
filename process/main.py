...
def cal_extrinsic_from_two_box_object_list(box_object_list1, box_object_list2, filter_num = 15, verbose = False):
    ...
    if matches == []:
        return None
    matches2extrinsics = Matches2Extrinsics(box_object_list1, box_object_list2, matches_score_list=matches_with_score_list, verbose=verbose, max_matches_num=boxes_match.get_max_matches_num())
...
def batching_test_extrisic_from_two_box_object_list(verbose = False, filter_num = 15):
    ...
    for infra_file_name, vehicle_file_name, infra_boxes_object_list, vehicle_boxes_object_list, T_true in reader.generate_infra_vehicle_bboxes_object_list():
        ...
        if True:
            ...
            result = cal_extrinsic_from_two_box_object_list(filtered_infra_boxes_object_list, filtered_vehicle_boxes_object_list, filter_num=filter_num, verbose=verbose)
...
# def cal_extrinsic_from_two_box_object_list_image(box_object_list1, box_object_list2, filter_num = 15, verbose = False, image_list = None):
#     boxes_match = BoxesMatch(box_object_list1, box_object_list2, image_list = image_list)
#     matches = boxes_match.get_matches()
#     if matches == []:
#         return None
#     pso = PSO_Executor(box_object_list1, box_object_list2, matches=matches, filter_num=filter_num, verbose=verbose, turn_off_pso=False)
...
def batching_test_extrisic_from_two_box_object_list_appearance(verbose = False, filter_num = 15):
    reader = CooperativeBatchingReader('config.yml')
    cnt = 0 
    ...
    #image
    for infra_file_name, vehicle_file_name, infra_boxes_object_list, vehicle_boxes_object_list, infra_image, vehicle_image, T_true in reader.generate_infra_vehicle_bboxes_object_list_image():
        ...
        if True:
            ...
            filtered_infra_boxes_object_list = Filter3dBoxes(infra_boxes_object_list).filter_according_to_size_topK(filter_num)
            filtered_vehicle_boxes_object_list = Filter3dBoxes(vehicle_boxes_object_list).filter_according_to_size_topK(filter_num)
            result = cal_extrinsic_from_two_box_object_list_image(filtered_infra_boxes_object_list, filtered_vehicle_boxes_object_list, filter_num=filter_num, verbose=verbose, image_list=(infra_image, vehicle_image))
