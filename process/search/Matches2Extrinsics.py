...
class Matches2Extrinsics:
    ...
    def __init__(self, infra_boxes_object_list, vehicle_boxes_object_list, true_T_6DOF = None, matches_score_list = [], verbose = False, max_matches_num = 10, true_matches = []):
    ...
    def get_single_extrinsic(self):
        match = self.matches_score_list[0][0]
        infra_box_object = self.infra_boxes_object_list[match[0]]
        vehicle_box_object = self.vehicle_boxes_object_list[match[1]]
        return convert_T_to_6DOF(get_extrinsic_from_two_3dbox_object(infra_box_object, vehicle_box_object))
    
    def get_combined_extrinsic_using_optimization(self):
        adequete_matches = self.cal_adequete_matches_true_value()
        infra_boxes_object_list = [self.infra_boxes_object_list[match[0]] for match in adequete_matches]
        vehicle_boxes_object_list = [self.vehicle_boxes_object_list[match[1]] for match in adequete_matches]
        
        return convert_T_to_6DOF(optimize_extrinsic_from_two_mixed_3dbox_object_list(infra_boxes_object_list, vehicle_boxes_object_list))
