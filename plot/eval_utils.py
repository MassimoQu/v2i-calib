...
def compute_type(gt_annos, pred_annos, cla, iou_threshold, view='3d'):
    """
    Input:
    ...
    for gt_anno in gt_annos:
        ...
        for i in range(len(pred_annos)):
            ...
            try:
                ...
                iou, iou_2d = box3d_iou(gt_anno["box"][perm_label], pred_anno["box"][perm_label])
                # iou, iou_2d = box3d_iou(gt_anno["box"], pred_anno["box"])
                # if iou != 0:
                #     print(iou)
            except Exception:
                iou, iou_2d = 0, 0
    ...
    # if num_tp != 0:
    #     print("num_tp: {}, pred: {}, gt: {}".format(num_tp, p, q))
...
# TODO: add calibration evaluation
class CalibEvaluator(object):
    # attention to the box point sequence
    def __init__(self, pred_classes=['car']):
        self.pred_classes = pred_classes
        self.all_preds = {}
        self.gt_num = {}
        for pred_class in self.pred_classes:
    ...
    def add_frame(self, pred, label):
        for pred_class in self.pred_classes:
            for iou in iou_threshold_dict[pred_class]:
                pred_result, num_label, num_tp = compute_type(label, pred, pred_class, iou)  # test
                self.all_preds[pred_class][iou] += pred_result
                self.gt_num[pred_class][iou] += num_label
                # logger.debug("iou: {}, tp: {}, all_pred: {}".format(iou, num_tp, len(pred["labels_3d"])))
                # print("iou: {}, tp: {}, all_pred: {}".format(iou, num_tp, len(pred["labels_3d"])))

    # def print_ap(self, type="micro"):
    #     for pred_class in self.pred_classes:
    #         for iou in iou_threshold_dict[pred_class]:
    #             ap = compute_ap(self.all_preds[pred_class][iou], self.gt_num[pred_class][iou])
    #             print("%s IoU threshold %.2lf, Average Precision = %.2lf" % (pred_class, iou, ap * 100))

    def add_frame_BBox3d_format(self, pred, label):
        self.add_frame(convert_bbox3d_object_list_to_v2xsim_format(pred), convert_bbox3d_object_list_to_v2xsim_format(label))
    ...
    def get_mAP(self):
        ...
        for iou in [0.3, 0.5, 0.7]:
            ...
            for pred_class in self.pred_classes:
                ap = compute_ap(self.all_preds[pred_class][iou], self.gt_num[pred_class][iou])
                # for pred in self.all_preds[pred_class][iou]:
                #     print('id:', pred['id'])
                #     print('score:', pred['score'])
                #     print('type:', pred['type'])
                # print('self.gt_num[pred_class][iou]', self.gt_num[pred_class][iou])
                # print("%s IoU threshold %.2lf, Average Precision = %.2lf" % (pred_class, iou, ap * 100))
                result[iou] += ap
            result[iou] /= len(self.pred_classes)
