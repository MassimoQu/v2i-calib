...
def plot_curve_oIoU_under_trueT_with_bias(infra_id, vehicle_id, cnt, step):
    ...
    for i in range(-cnt - 1, cnt + 1):
        for key in key_list:
            ...
            oIoU = CorrespondingDetector(converted_infra_bbox_list, vehicle_bbox_list, corresponding_strategy='iou').get_Yscore()
...
def plot_curve_overall_distance_under_trueT_with_bias(infra_id, vehicle_id, cnt, step):
    ...
    for i in range(-cnt - 1, cnt + 1):
        for key in key_list:
            converted_infra_bbox_list = implement_T_3dbox_object_list(convert_6DOF_to_T(extrinsic_unit[key] * i), infra_bbox_list)
            odistance = (CorrespondingDetector(converted_infra_bbox_list, vehicle_bbox_list, corresponding_strategy='centerpoint_distance').get_Yscore() + CorrespondingDetector(converted_infra_bbox_list, vehicle_bbox_list, corresponding_strategy='vertex_distance').get_Yscore()) / 2
...
def plot_separate_motion_metrics(infra_id, vehicle_id, cnt, step):
    ...
    for i in range(-cnt, cnt + 1):
        for key in key_list:
            converted_infra_bbox_list = implement_T_3dbox_object_list(convert_6DOF_to_T(extrinsic_unit[key] * i), infra_bbox_list)
            oIoU = CorrespondingDetector(converted_infra_bbox_list, vehicle_bbox_list, corresponding_strategy='iou').get_Yscore()
            oDistance = (CorrespondingDetector(converted_infra_bbox_list, vehicle_bbox_list, corresponding_strategy='centerpoint_distance').get_Yscore() +
                         CorrespondingDetector(converted_infra_bbox_list, vehicle_bbox_list, corresponding_strategy='vertex_distance').get_Yscore()) / 2
            oIoU_data[key].append(oIoU)
            oDistance_data[key].append(oDistance)
...
def plot_curve_oIoU_under_trueT_different_scene(frame_cnt = 100):
    ...
    for infra_file_name, vehicle_file_name, infra_bboxes_object_list, vehicle_bboxes_object_list, T_i2v in batch_reader.generate_infra_vehicle_bboxes_object_list(start_idx=0, end_idx=-1):
        ...
        for key in key_list:
            ...
            oIoU = CorrespondingDetector(converted_infra_bbox_list, vehicle_bboxes_object_list, corresponding_strategy='iou').get_Yscore()
            oIoU_list[key].append(oIoU)
